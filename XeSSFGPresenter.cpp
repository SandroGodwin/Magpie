...
struct XeSSFGPresenter::Impl {
	...
	// ClearUnorderedAccessViewFloat requires a CPU handle from a
	// non-shader-visible heap plus the matching GPU handle from a
	// shader-visible heap copy. A single shader-visible heap used for
	// both (as before) is undefined behaviour per the D3D12 spec and
	// crashes (0xc0000005) on some drivers/GPUs, e.g. GTX 1050 Ti.
	winrt::com_ptr<ID3D12DescriptorHeap> clearCpuHeap12;
	winrt::com_ptr<ID3D12DescriptorHeap> clearGpuHeap12;
...
static bool CreateFlatResource(
	...
	// Create the UAV in the non-shader-visible heap, then mirror it into
	// the shader-visible heap so Clear gets a CPU handle it can safely
	// dereference and a GPU handle that matches the bound heap.
	impl.device12->CreateUnorderedAccessView(resource.get(), nullptr, &uav, cpu);
	impl.device12->CopyDescriptorsSimple(
		1, gpuHeapCpu, cpu, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
...
static bool CreateSizeDependentResources(XeSSFGPresenter::Impl& impl) noexcept {
	...
	HRESULT hr = impl.device12->CreateDescriptorHeap(
		&cpuHeapDesc, IID_PPV_ARGS(impl.clearCpuHeap12.put()));
	if (FAILED(hr)) {
		Logger::Get().ComError("Create XeSSFG CPU clear descriptor heap failed", hr);
		return false;
	}

	D3D12_DESCRIPTOR_HEAP_DESC gpuHeapDesc{};
	gpuHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
	gpuHeapDesc.NumDescriptors = 2;
	gpuHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
	hr = impl.device12->CreateDescriptorHeap(
		&gpuHeapDesc, IID_PPV_ARGS(impl.clearGpuHeap12.put()));
	if (FAILED(hr)) {
		Logger::Get().ComError("Create XeSSFG GPU clear descriptor heap failed", hr);
		return false;
	}
	...
	ID3D12DescriptorHeap* heaps[]{ impl.clearGpuHeap12.get() };
	impl.commandList12->SetDescriptorHeaps(1, heaps);
	const UINT stride = impl.device12->GetDescriptorHandleIncrementSize(
		D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
	D3D12_CPU_DESCRIPTOR_HANDLE cpu =
		impl.clearCpuHeap12->GetCPUDescriptorHandleForHeapStart();
	D3D12_CPU_DESCRIPTOR_HANDLE gpuHeapCpu =
		impl.clearGpuHeap12->GetCPUDescriptorHandleForHeapStart();
	D3D12_GPU_DESCRIPTOR_HANDLE gpu =
		impl.clearGpuHeap12->GetGPUDescriptorHandleForHeapStart();
	...
	if (!CreateFlatResource(impl, DXGI_FORMAT_R16G16_FLOAT,
		impl.zeroMotion12, cpu, gpuHeapCpu, gpu, ZERO, barriers[0])) {
		return false;
	}
	cpu.ptr += stride;
	gpuHeapCpu.ptr += stride;
	gpu.ptr += stride;
	if (!CreateFlatResource(impl, DXGI_FORMAT_R32_FLOAT,
		impl.flatDepth12, cpu, gpuHeapCpu, gpu, FLAT_DEPTH, barriers[1])) {
		return false;
	}
...
static void ReleaseSizeDependentResources(XeSSFGPresenter::Impl& impl) noexcept {
	...
	impl.clearCpuHeap12 = nullptr;
	impl.clearGpuHeap12 = nullptr;
}
